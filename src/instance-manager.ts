...
export class InstanceManager {
  ...
  addInstance(instanceData: InstanceData): number {
    if (this.instances.length >= this.maxInstances) {
      throw new EngineError(`Maximum instances exceeded: ${this.maxInstances}`, 'MAX_INSTANCES_EXCEEDED');
    }
  ...
  private updateBatches(): void {
    ...
    const batchGroups: Array<{ key: string; meshId: string; materialId: number; instanceIndices: number[] }> = [];
  ...
  getInstanceTransformBuffer(): GPUBuffer {
    if (!this.instanceTransformBuffer) {
      throw new EngineError('Instance transform buffer not initialized', 'BUFFER_NOT_INITIALIZED');
    }
  ...
  getInstanceMetadataBuffer(): GPUBuffer {
    if (!this.instanceMetadataBuffer) {
      throw new EngineError('Instance metadata buffer not initialized', 'BUFFER_NOT_INITIALIZED');
    }
