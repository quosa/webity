...
export class RigidBody extends Component {
    ...
    private syncToWasm(): void {
        if (this.wasmEntityIndex < 0) {
            console.log(
                `⚠️ RigidBody.syncToWasm() for "${this.gameObject?.name}": no WASM entity index`
            );
            return;
        }
        ...
        if (!scene) {
            console.log(`⚠️ RigidBody.syncToWasm() for "${this.gameObject?.name}": no scene`);
            return;
        }
        ...
        if (transform) {
            ...
            // IMPORTANT: This sync assumes stable WASM entity indices!
            // If we implement entity reordering optimizations (for rendering batching),
            // we'll need to either:
            // 1. Update GameObject→WASM index mapping when reordering, OR
            // 2. Use a stable entity ID system instead of array indices
            ...
            // Sync rotation to WASM for visual rendering
            const worldRot = transform.getWorldRotation();
            scene.updateWasmEntityRotation(this.wasmEntityIndex, worldRot);

            // Debug logging occasionally
            if (Math.random() < 0.01) {
                console.log(
                    `🔄 RigidBody.syncToWasm() for "${this.gameObject?.name}" entity ${this.wasmEntityIndex}: pos=(${worldPos.x.toFixed(1)}, ${worldPos.y.toFixed(1)}, ${worldPos.z.toFixed(1)}), vel=(${this.velocity.x.toFixed(1)}, ${this.velocity.y.toFixed(1)}, ${this.velocity.z.toFixed(1)})`
                );
            }
