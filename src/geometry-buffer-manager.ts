...
export class GeometryBufferManager {
  ...
  private async updateVertexBuffer(): Promise<void> {
    ...
    if (vertices.length > 0) {
      this.device.queue.writeBuffer(this.combinedVertexBuffer, 0, vertices.buffer, vertices.byteOffset, vertices.byteLength);
    }
  }
  ...
  private async updateIndexBuffer(): Promise<void> {
    ...
    if (indices.length > 0) {
      this.device.queue.writeBuffer(this.combinedIndexBuffer, 0, indices.buffer, indices.byteOffset, indices.byteLength);
    }
  }
  ...
  private async updateMaterialBuffer(): Promise<void> {
    ...
    if (materialData.length > 0) {
      this.device.queue.writeBuffer(this.materialDataBuffer, 0, materialData.buffer, materialData.byteOffset, materialData.byteLength);
    }
  }
  ...
  drawMesh(renderPass: any, meshId: string, instanceCount: number, firstInstance = 0): void { // GPURenderPassEncoder
  ...
  getStats(): { vertexCount: number; indexCount: number; meshCount: number; materialCount: number } {
    return {
