...
export class Engine implements GameEngine {
    ...
    // Scene management
    private currentScene: import('./scene.js').Scene | undefined = undefined;
    ...
    async loadAssets(assets: AssetConfig): Promise<void> {
        ...
        if (assets.ball) {
            // Use the new function that accepts both segments and radius
            this.wasm.generate_sphere_mesh_with_radius(
                assets.ball.segments,
                assets.ball.radius || 0.5
            );
        }
    ...
    setCurrentScene(scene: import('./scene.js').Scene | null): void {
        if (scene) {
            this.currentScene = scene;
        } else {
            this.currentScene = undefined;
        }
        console.log(`🎬 Engine.setCurrentScene(): ${scene ? scene.name : 'null'}`);
    }

    getCurrentScene(): import('./scene.js').Scene | undefined {
    ...
    setWasmEntityPosition(index: number, x: number, y: number, z: number): void {
        ...
        // Always log position changes to debug the z=-10 issue
        console.log(
            `🎯 Engine.setWasmEntityPosition(${index}, ${x.toFixed(1)}, ${y.toFixed(1)}, ${z.toFixed(1)})`
        );
    ...
    setWasmEntityRotation(index: number, x: number, y: number, z: number): void {
        if (!this.wasm || index < 0) return;

        // Convert degrees to radians for WASM
        const rx = (x * Math.PI) / 180;
        const ry = (y * Math.PI) / 180;
        const rz = (z * Math.PI) / 180;
        this.wasm.set_entity_rotation(index, rx, ry, rz);

        if (Math.random() < 0.01) {
            // Debug log occasionally
            console.log(
                `🔄 Engine.setWasmEntityRotation(${index}, ${x.toFixed(1)}°, ${y.toFixed(1)}°, ${z.toFixed(1)}°)`
            );
        }
    }
    ...
    private gameLoop = async (): Promise<void> => {
        ...
        try {
            // Update Scene (GameObjects) first
            ...
            if (
                entityCount % 50 === 0 &&
                Math.floor(performance.now() / 5000) !== Math.floor((performance.now() - 16) / 5000)
            ) {
            ...
            await this.renderer.renderUnified(this.wasm!.memory.buffer, uniformOffset, this.wasm!);
        } catch (error) {
