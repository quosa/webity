...
export class Scene {
    ...
    private spawnWasmEntities(): void {
        ...
        const meshTypes = new Set(
            renderableObjects.map(obj => obj.getComponent(MeshRenderer)!.getMeshType())
        );
        if (meshTypes.size > 1) {
            console.log(
                '🎨 Mixed mesh types detected! Using multi-mesh renderer for optimal rendering.'
            );
            console.log(
                '    Mesh types in scene:',
                Array.from(meshTypes).map(t => (t === 0 ? 'SPHERE' : 'CUBE'))
            );
        }
        ...
        for (const gameObject of renderableObjects) {
            ...
            console.log(
                `🎮 Spawning ${gameObject.name} at position (${worldPos.x}, ${worldPos.y}, ${worldPos.z})`
            );
            ...
            if (rigidBody) {
                ...
                console.log(
                    `🎮 RigidBody velocity for "${gameObject.name}": (${velocity.x}, ${velocity.y}, ${velocity.z})`
                );
                if (velocity.x !== 0 || velocity.y !== 0 || velocity.z !== 0) {
                    this.engine.setWasmEntityVelocity(
                        wasmEntityIndex,
                        velocity.x,
                        velocity.y,
                        velocity.z
                    ...
                    console.log(
                        `🎮 Set WASM entity ${wasmEntityIndex} velocity to: (${velocity.x}, ${velocity.y}, ${velocity.z})`
                    );
                } else {
                    console.log(
                        `🎮 No velocity to set for "${gameObject.name}" (all components are 0)`
                    );
                }
            }

            console.log(
                `🎮 Spawned ${meshRenderer.getMeshType() === MeshType.CUBE ? 'cube' : 'sphere'} entity ${wasmEntityIndex} for GameObject "${gameObject.name}"`
            );
        }
    }
    ...
    private setupGameObjectWasmIntegration(gameObject: GameObject): void {
        ...
        console.log(
            `🎮 Setting up WASM integration for GameObject "${gameObject.name}" (no spawning)`
        );
    }
        ...
        for (const gameObject of rigidBodyObjects) {
            ...
            if (wasmIndex >= 0 && transform) {
                ...
                if (!rigidBody.isKinematic) {
                    ...
                    const worldPos = transform.getWorldPosition();
                    this.engine.setWasmEntityPosition(
                        wasmIndex,
                        worldPos.x,
                        worldPos.y,
                        worldPos.z
    ...
    updateWasmEntityRotation(
        wasmIndex: number,
        rotation: { x: number; y: number; z: number }
    ): void {
        if (this.engine && wasmIndex >= 0) {
            this.engine.setWasmEntityRotation(wasmIndex, rotation.x, rotation.y, rotation.z);
        }
    }
