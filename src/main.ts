...
async function startDemo(): Promise<void> {
    ...
    */
        ...
        fancyDemoButton?.addEventListener('click', () => {
            ...
            console.log(
                `🎪 Fancy demo scene activated! ${engine.getWasmEntityCount()} entities total`
            );
        });
        ...
        singleBallButton?.addEventListener('click', () => {
            ...
            console.log(
                `🎯 Single ball test: ${engine.getWasmEntityCount()} ball for physics testing`
            );
        });

        const collisionTestButton = document.getElementById(
            'collisionTestButton'
        ...
        collisionTestButton?.addEventListener('click', () => {
            ...
            console.log(
                `⚡ Collision test: ${engine.getWasmEntityCount()} balls for collision testing`
            );
        });
        ...
        rainButton?.addEventListener('click', () => {
            if (activeRainSystem && activeRainSystem.isActive()) {
                ...
                console.log(
                    `🌧️ Rain scene started with ${activeRainSystem.getCurrentBallCount()} initial balls`
                );
            }
        });
        ...
        cubeStackButton?.addEventListener('click', () => {
            try {
                ...
                console.log(
                    `🎾 Entity count after GameObject scene: ${engine.getWasmEntityCount()}`
                );
            } catch (error) {
                console.error('Failed to create cube stack scene:', error);
                showError(
                    `GameObject scene error: ${error instanceof Error ? error.message : 'Unknown error'}`
                );
            }
        });
        ...
        debugDisplay.style.cssText =
            'position: absolute; top: 120px; left: 10px; color: red; font-family: monospace; font-size: 12px; background: rgba(0,0,0,0.8); padding: 5px; border-radius: 3px; max-width: 300px;';
...
function checkFloatingEntities(engine: Engine): void {
    ...
    if (floatingIndex !== 10000) {
        // MAX_ENTITIES is 10000, used as sentinel
      ...
      `;
        ...
        console.warn(
            `🐛 Floating entity detected: #${floatingIndex} at Y=${posY.toFixed(3)}, velY=${velY.toFixed(6)}`
        );
