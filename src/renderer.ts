...
export class Renderer {
    ...
    async initUnifiedRenderer(wasm: any): Promise<void> {
        ...
        this.unifiedRendererBridge = new UnifiedRendererBridge(this.device, this.context, wasm);
    ...
    async renderMultipleEntitiesInstanced(
        ...
        console.warn(
            '⚠️ renderMultipleEntitiesInstanced is deprecated. Use renderUnified() instead.'
        );
        await this.renderUnified(wasmMemory, uniformOffset, wasm);
    }

    async renderMixedMeshesInstanced(
        wasmMemory: ArrayBuffer,
        uniformOffset: number,
        wasm: any
    ): Promise<void> {
