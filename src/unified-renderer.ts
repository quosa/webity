...
export class UnifiedRenderer {
  ...
  render(): void {
    ...
    // Begin render pass
    const commandEncoder = this.device.createCommandEncoder({ label: 'Unified Render Commands' });
  ...
  private renderDirect(renderPass: any, batches: RenderBatch[]): void { // GPURenderPassEncoder
  ...
  private renderIndirect(renderPass: any, batches: RenderBatch[]): void { // GPURenderPassEncoder
    ...
    if (this.indirectDrawCommands.length > 0) {
      this.device.queue.writeBuffer(this.indirectDrawBuffer!, 0, this.indirectDrawCommands.buffer, this.indirectDrawCommands.byteOffset, this.indirectDrawCommands.byteLength);
